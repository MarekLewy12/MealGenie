...
export function MealCard({ meal, onSelect }: MealCardProps) {
  ...
  return (
    <div className="group flex h-full flex-col gap-4 rounded-2xl border border-slate-200 bg-white/90 p-6 shadow-xl shadow-indigo-100/60 transition hover:-translate-y-1 hover:border-indigo-400/50 hover:shadow-indigo-200/70 dark:border-slate-800 dark:bg-slate-900/80 dark:shadow-slate-950/40 dark:hover:border-indigo-500/50 dark:hover:shadow-indigo-900/30">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{meal.name}</h3>
          <p className="mt-2 text-sm text-slate-700 dark:text-slate-300">{meal.description}</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white/80 p-4 dark:border-slate-800 dark:bg-slate-800/60">
        ...
        <ul className="space-y-2 text-sm text-slate-700 dark:text-slate-200">
          ...
          {remainingCount > 0 && (
            <li className="text-sm text-slate-500 dark:text-slate-400">+ {remainingCount} więcej</li>
          )}
        </ul>
      </div>
